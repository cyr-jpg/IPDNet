...
class RepASConv7x7(nn.Module):
    ...
    def _build_branch2(self):
        return nn.Sequential(
            nn.Conv2d(self.in_channels, self.in_channels, 1, stride=self.stride, bias=False),
            nn.BatchNorm2d(self.in_channels, eps=1e-5),
            nn.SiLU(),
            nn.Conv2d(self.in_channels, self.in_channels, (1, 7), stride=1, padding=(0, 3), bias=False),
            nn.BatchNorm2d(self.in_channels, eps=1e-5),
            nn.SiLU(),
            nn.Conv2d(self.in_channels, self.in_channels, (7, 1), stride=1, padding=(3, 0), bias=False),
            nn.BatchNorm2d(self.in_channels, eps=1e-5),
            nn.SiLU(),
            nn.Conv2d(self.in_channels, self.out_channels, 1, stride=1, bias=False),
            nn.BatchNorm2d(self.out_channels, eps=1e-5)
        )
